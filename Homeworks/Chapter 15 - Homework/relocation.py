...
#my code
parser.add_option('-g', '--graph',     default=False,     help='make a graph with x random inputs',              action='store_true',      dest='graph')
...
while(uplimit >= 0):
    ...
    validacc = 0 #total valid for graph fraction of valid/total
    for i in range(0,options.num):
        vaddr = int(asize * random.random())
        if options.solve == False and options.graph == False:
            ...
            if (vaddr >= limit):
                ...
                validacc = validacc + 1
    print("test" + str(validacc))
    dataset.append([limit, float(validacc/options.num)])   #then x, y coordinate for the graph
    print('')
